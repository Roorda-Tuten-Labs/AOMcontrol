...
if SYSPARAMS.aoms_state(2) == 1
    ...
    elseif Mov.aom1seq(end) ~= 0 && Mov.aom1seq(end) ~= 1
        if locupdate || (Mov.aom1seq(end) ~= Mov.aom1seq(end-1))
            ...
            temp_im_d = ((temp_im_d-min(min(temp_im_d)))*((255-51)/((max(max(temp_im_d))-min(min(temp_im_d)))+1)));
            temp_im_dim = size(temp_im_d);
            CurFrame(floor(256+locy+StimParams.aomoffs(1,2)+Mov.aom1offy(end)-temp_im_dim(1)/2)+1:floor(256+locy+StimParams.aomoffs(1,2)+Mov.aom1offy(end)-temp_im_dim(1)/2)+temp_im_dim(1),floor(256+locx+StimParams.aomoffs(1,1)+Mov.aom1offx(end)-temp_im_dim(2)/2)+1:floor(256+locx+StimParams.aomoffs(1,1)+Mov.aom1offx(end)-temp_im_dim(2)/2)+temp_im_dim(2),1) = CurFrame(floor(256+locy+Mov.aom1offy(end)-temp_im_dim(1)/2)+1:floor(256+locy+Mov.aom1offy(end)-temp_im_dim(1)/2)+temp_im_dim(1),floor(256+locx+Mov.aom1offx(end)-temp_im_dim(2)/2)+1:floor(256+locx+Mov.aom1offx(end)-temp_im_dim(2)/2)+temp_im_dim(2),1)+uint8(temp_im_d);
            update = 1;
        end
