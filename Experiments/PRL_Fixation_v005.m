...
%               modified from 'Sam5CondFixation...v072'&'TumblingE_tremor.m'
%               School of Optometry - University of California, Berkeley
%               v0.01: version 1: resetting only once at beginning the
%                      stimuli location
%               v0.02: tests of various stimulus dimensions
%               v0.03: addition of the square box/hori lines.
%               v005 : -correction of the pseudoRand that was not pseudoRand
%                      -removal of createStimulus called for each trial
% =======================================================================
...
if vid_dur <44%35%30%36
    fprintf('Error: too short duration for 21 repetitions*2sec=42sec\n')
    return
end
...
%}
fixSeq=repmat(1:7,3);% & steps
for i=1:5
    vec=randperm(21);% 21 trials
    pRandSeq(i,:)=fixSeq(vec);
end
pseudoRandSteps=pRandSeq;
...
message1 = ['PRL Experiment - Condition' condition 'Session' num2str(session) ' Trial ' num2str(trial) ' of ' num2str(ntrials) ];
message = sprintf('%s\n%s', message1);
%Mov.msg = message;
...
grayValue=0;
%TO BE REMOVED
indices=createStimulus(gap,grayValue);
...
%             end
            ...
            if gamePad.buttonStart
                ...
                state = 'stimulus';
                condition = 'V';%only used here for writing into condition file
                
                command = ['LocUser#256#256#'];
        ...
        case {'stimulus'} % Present the stimulus
            ...
            if UpdateStimulus == 1 % Update the stimulus for the current trial, present and record
                ...
                while length(sequenceVector)<length(aom0seq)
                    %fixed interval of 1 sec.=>presentation every 1sec
                    intervalLength = 60;
                    % sequenceVector for Red was 2,[3,4,5,6,7,8,9]
                    
                    % 2 to load fix squared box
                    sequenceVector = [sequenceVector 2.*ones(1,intervalLength)]; %#ok<AGROW>; interval
                    %sequenceVectorG= [sequenceVectorG   zeros(1,intervalLength)];
                    xOffsetVector =  [xOffsetVector  2.*ones(1,intervalLength)]; %#ok<AGROW>; zeros at interval
                    yOffsetVector =  [yOffsetVector  2.*ones(1,intervalLength)]; %#ok<AGROW>

                    randN=pseudoRandSteps(trial,StimNumber);%where StimNumber increment from 1 to ? 28
                    stepSize=[1 2 3 4 5 6 7];% 7 steps
                    randSize=stepSize(randN);
                    timSampStimOnset = length(sequenceVector);
                    
                    % randsize start from 3, to 9 for red
                    sequenceVector = [sequenceVector ones(1,2)*(randSize+2)];%2 for 2 frame duration,15 for 0.5sec duration
                    %stimulus is on
                    xOffsetVector =  [xOffsetVector ones(1,2)*randi([-10 10])];  %#ok<AGROW>
                    yOffsetVector =  [yOffsetVector ones(1,2)*randi([-10 10])];  %#ok<AGROW>
                    
                    conditionMat = [conditionMat ; 5, session,trial, randSize,(timSampStimOnset+1)*0.033, timSampStimOnset+1];
                    % '5' for condition 'V' for consistency with before
                    if StimNumber<21
                        StimNumber=StimNumber+1;
                    else
                        sequenceVector(end+1:length(aom0seq)) = 0;
                    end
                end
                if length(sequenceVector)>length(aom0seq);
                    sequenceVector(length(aom0seq)+1:end) = [];
                    %sequenceVectorG(length(aom0seq)+1:end) = [];
                end
                %%
                ...
                % ! you need to reverse the IR and Red channels on the
...
%                 end
                ...
                message1 = ['PRL - Vernier Session:' num2str(session) ' Trial ' num2str(trial) ' of ' num2str(ntrials)];
                message = sprintf('%s\n%s', message1);
                Mov.msg = message;
                set(handles.aom1_state, 'String', message);
           ...
           %     gap = CFG.stim_size; % is the gap between the prongs of the E in pixels ...
            ...
            while GetSecs-startTime< (length(sequenceVector)/30)%=36
                ...
                if gamePad.buttonY
                    ...
                    finalRespMat = [finalRespMat;[5, session,trial , resp , GetSecs-startTime, round((GetSecs-startTime)*30)]];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%code to generate E%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [indices]=createStimulus(gap,grayValue)
% gap to change to 2 to easier % a single pixel bar height for the Vernier
...
temp=[];
        ...
        % Sam XP: bars are 3 pix height x 47 pix width, so 0.3'/18" x 5'
        % PRL XP: bars are 9 pix height x 19 pix width, so 1'x2'

        % 2' width x1'height
        hB=9;
        wB=19;
        hbar=ones(1*hB,1*wB)*(1-grayValue/255);%half bar
        ...
        % horizontal separation of 19pix=2min width,9pix=1min
        sB=9;%
        vmiddle=[zeros(3*gap,sB); zeros(hB,sB);zeros(3*gap,sB)];
        %%[padt; zeros(19,19) ;padt];
        
        %cd([pwd,filesep,'tempStimulus']);
        % RED aom0seq and aom1seq from 2 to 9 / GREEN: 0 (blank), 10 (central dot)
        
        % write RED 1 frame for squared box
        hS=120; %length of the squared box,120<=>12.65min
        wS=240;% 
        %
        hlS=3;   %height of the line of the box
        R =uint8(255.*ones(hS,wS));
        % ---- rectangle ----------
 %       R(1:hlS,:)=0;R(end-hlS+1:end,:)=0;
 %       R(:,1:hlS)=0;R(:,end-hlS+1:end)=0;
        %----- rectangle corners
        %each corner width or height is 20 pix, so 2min
        R(1:hlS,1:20)=0;
        R(1:hlS,end-20:end)=0;
        R(end-hlS+1:end,1:20)=0;
        R(end-hlS+1:end,end-20:end)=0;
        R(1:20,1:hlS)=0;
        R(end-20:end,1:hlS)=0;
        R(1:20,end-hlS+1:end)=0;
        R(end-20:end,end-hlS+1:end)=0;
        imwrite(R,['frame2.bmp']);
        % write RED 7 frames with 
        for i=1:7 %from -3 to +3
            ...
            V=[vleft vmiddle vright];
            padh=zeros(1,size(V,2));padv=zeros(size(V,1)+2,1);
            V=[padh;V;padh];
            V=[padv V padv];
            V=1-V;
            
            % frame contain border B + Vernier target V
            %F=uint8(ones(512,512));
            R =uint8(255.*ones(hS,wS));
            % add thin black lines always at the same position in the frame
            % to be inpainted after with noise 
            % ---- rectangle --------
 %           R(1:hlS,:)=0;
 %           R(end-hlS+1:end,:)=0;
 %           R(:,1:hlS)=0;
 %           R(:,end-hlS+1:end)=0;
            ...
            R( size(R,1)/2-floor(size(V,1)/2):...
               size(R,1)/2+floor(size(V,1)/2),...
               size(R,2)/2-floor(size(V,2)/2):...
               size(R,2)/2+floor(size(V,2)/2))=255.*V;
            imwrite(R,['frame' num2str(i+2) '.bmp']);
            temp = 2:9; 
        end
        % WRITE empty green frame
        %{
        G =uint8(255.*ones(hS,wS));
        %add a beep at few central pixels;
        G(140:142,size(R,1)/2-floor(size(V,1)/2):size(R,1)/2+floor(size(V,1)/2))=0;
        imwrite(G,['frame10.bmp']);
        %}
        indices=temp;
...
% ------------------- for diagonals ---------------------
            %142 is 15' eccentricity, 
            %95 is 10' eccentricity
            %{
            ecc=142;
            for j=1:110
               V(256+ecc+j,256+ecc+j)=0;
               V(256+ecc+j,256-ecc-j)=0;
               V(256-ecc-j,256+ecc+j)=0;
               V(256-ecc-j,256-ecc-j)=0;
               
               V(256+ecc-1+j,256+ecc+j)=0;% 1 pix lower vertically
               V(256+ecc-1+j,256-ecc-j)=0;
               V(256-ecc-1-j,256+ecc+j)=0;
               V(256-ecc-1-j,256-ecc-j)=0;
            end
            %}
